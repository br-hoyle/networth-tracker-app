...
def display_recent_balance_changes(conn):
    """Render a dashboard section showing recent balance changes by institution and category."""
    ...
    with stylable_container(
        ...
        recent_dates = (
            df["full_date"].drop_duplicates().sort_values().iloc[-user_selected:]
        )
        df_filtered = df[df["full_date"].isin(recent_dates)]

        # Pivot the table to show balances across dates
        ...
        # Re-identify only balance columns (excluding change columns)
        balance_columns = [
            col for col in pivot_df.columns if col not in ["Last Change", "Change"]
        ]

        # Add sparkline-style balance history column
        ...
        def highlight_change(row, col):
            """Return CSS style for change indicators based on improvement or decline."""
            ...
            if improved:
                return f"background-color: {get_config_value('theme.ColorPalette.green')}; color: #2c6a3a"
            else:
                return f"background-color: {get_config_value('theme.ColorPalette.red')}; color: #76333c"

        def style_table(df):
            """Apply custom styles to a dataframe."""
        ...
        for category, table in tables.items():
            ...
            first_date_col, prev_date_col, last_date_col = (
                num_date_cols[0],
                num_date_cols[-2],
                num_date_cols[-1],
            )
